...
function showAction($entityID) {
	global $alreadygot, $admtext, $albumID, $dims;
	
	$lines = "<tr id=\"linkrow_$entityID\"><td class=\"lightback\">";
	$lines .= "<div id=\"link_$entityID\" class=\"normal\" style=\"text-align:center;width:50px;";
	if($albumID) {
		$gotit = in_array($entityID,$alreadygot);
		if($gotit)
			$lines .= "display:none";
		$lines .= "\"><a href=\"#\" onclick=\"return addAlbum2EntityLink(document.find, '$entityID');\">" . $admtext['add'] . "</a></div>";
		$lines .= "<div id=\"linked_$entityID\" style=\"text-align:center;width:50px;";
		if(!$gotit)
			$lines .= "display:none";
		$lines .= "\"><img src=\"tng_test.gif\" alt=\"\" $dims class=\"smallicon\">";
	}
	else {
		$lines .= "\"><a href=\"#\" onclick=\"selectEntity(document.find.newlink1, '$entityID');\">" . $admtext['select'] . "</a>";
	}
	$lines .= "</div>";
	$lines .= "</td>";
...
function doFamilies($husbname, $wifename) {
	...
	$query .= "husbpeople.personID as hpersonID, husbpeople.firstname as hfirstname, husbpeople.lnprefix as hlnprefix, husbpeople.lastname as hlastname, husbpeople.prefix as hprefix, husbpeople.suffix as hsuffix, husbpeople.nameorder as hnameorder FROM $families_table $joinonwife $joinonhusb WHERE $allwhere $allwhere2 ORDER BY hlastname, hlnprefix, hfirstname LIMIT $maxsearchresults";
	$result = mysql_query($query) or die ("$admtext[cannotexecutequery]: $query");
